...
export default function RefundsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'ikas' | 'manual'>('ikas');
  ...
  const fetchRefunds = useCallback(async (currentToken: string) => {
    try {
      ...
      if (res.status === 200 && res.data?.data) {
        setRefunds(res.data.data as RefundWithOrder[]);
      } else {
    ...
    <div className="container mx-auto p-6">
      ...
      {activeTab === 'manual' ? (
        ...
        filteredRefunds.length === 0 && refunds.length > 0 ? (
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center">
            <p className="text-gray-500">Henüz manuel iade kaydı bulunmamaktadır.</p>
          </div>
        ) : (
